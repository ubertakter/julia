...
8. Place the starting and ending `"""` characters on lines by themselves.
   ...
   !!! note
       If using [`Documenter.jl`](https://github.com/JuliaDocs/Documenter.jl) to create documentation, do not
       have an empty line or comment between the end of the docstring and the function declaration. Doing so will
       result in `Documenter` not finding the docstring (and an associated `no doc found` error).
